...
namespace FractalExplorer.Forms.Fractals
{
    public partial class FractalRosslerForm : Form, ISaveLoadCapableFractal, IFullPreviewRenderCapableFractal, IHighResRenderable
    {
        ...
        private void ApplyDefaults()
        {
            ...
            ConfigureDecimal(_nudDt, 6, 0.0001m, 0.00001m, 0.2m, 0.01m);
            ...
            int? _ = null)
        {
            ...
            (double u, double v) Project(double px, double py, double pz)
            {
                ...
                {
                    RosslerProjection.XZ => (px, pz),
                    RosslerProjection.YZ => (py, pz),
                    _ => (px, py)
                };
            }
            ...
            for (int i = 0; i < warmup; i++)
            {
                ...
                StepRossler(ref x, ref y, ref z, a, b, c, dtD);
            }
        ...
        private static void StepRossler(ref double x, ref double y, ref double z, double a, double b, double c, double dt)
        {
            double dx = -y - z;
            double dy = x + a * y;
            double dz = b + z * (x - c);
        ...
        public void LoadState(FractalSaveStateBase state)
        {
            ...
            _nudDt.Value = Math.Max(_nudDt.Minimum, Math.Min(_nudDt.Maximum, rossler.Dt));
        ...
        public HighResRenderState GetRenderState()
        {
            return new HighResRenderState
            {
                ...
                FileNameDetails =
                    $"a{_nudA.Value:F3}_b{_nudB.Value:F3}_c{_nudC.Value:F3}_dt{_nudDt.Value:F4}_{GetSelectedProjection()}"
            };
        }
