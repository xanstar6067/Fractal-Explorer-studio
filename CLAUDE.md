...
| `Views/NewtonPaletteWindow.xaml` / `.xaml.cs` | Настройка цветов корней и палитр бассейнов Ньютона; вызывается из `NewtonPoolsWindow`, а с подписями корней или циклов и без флажка градиента — из `BasinExplorerWindow`. |
...
Ньютона, `basins` — только бассейны Мюллера, Лагерра, секущих, рациональных отображений и циклов
(несколько секунд):
