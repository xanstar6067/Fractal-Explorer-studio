...
| `Views/Ifs3DWindow.xaml` / `.xaml.cs` | Конструктор объёмных IFS: один режим с пресетами тетраэдра Серпинского, губки Менгера и объёмного папоротника. Редактор матриц 3×3, переносов и вероятностей сразу перестраивает стохастическую орбиту; облако точек можно поворачивать, двигать и масштабировать. Есть превью каталога, сохранения, точки интереса и экспорт. Связаны `Models/Ifs3DModels.cs`, `Core/Rendering/Ifs3DRenderer.cs`, `Infrastructure/Ifs3DSaveStore.cs`. |
...
| `Views/Fractal3DPaletteWindow.xaml` / `.xaml.cs` | Менеджер палитр трёхмерных фракталов: библиотека из двенадцати встроенных палитр и пользовательских (`Palettes\custom_palettes_fractal3d.json`), общая для всех семи видов. Редактор задаёт опорные цвета (до 16), плавный градиент или полосы, гамму и, по желанию, окружение — фон вверху и внизу, цвет источника света и цвет материала; окружение применяется только при включённом флажке, поэтому встроенные палитры свет и фон не трогают. Рядом с цветами идёт настоящий кадр фрактала с той же фигурой и того же ракурса, что открыты в окне. Вызывается из `Fractal3DWindow`. |
...
`fractal3d` — семь видов трёхмерных фракталов: дистанционные оценки на GPU не дают пустой кадр,
