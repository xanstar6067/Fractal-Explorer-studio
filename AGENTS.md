...
параметры формы, окраски и камеры доходят до шейдера, отмена и сохранения (нужен Direct3D 11), `cloud` — облачные
