...
Облачные сохранения: `Views/CloudSaveManagerWindow.xaml` / `.xaml.cs` — управление всеми режимами, двусторонняя ручная синхронизация, переименование и удаление; `CloudLoginWindow` — вход с DPAPI-сеансом, `CloudConflictWindow` — выбор при конфликте версий. Доступны из главного окна и общего менеджера сохранений. Клиент, доверие к корню Caddy, защищённый refresh token и индекс синхронизации находятся в `Infrastructure/Cloud/`, DTO — в `Models/CloudModels.cs`. Превью не передаются. Проверки: группа `cloud`; `cloud-live` выполняет только анонимные GET к VPS и проверяет TLS, без изменения данных.
