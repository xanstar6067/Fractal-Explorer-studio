...
`fractal3d` — десять трёхмерных режимов: дистанционные оценки на GPU не дают пустой кадр,
