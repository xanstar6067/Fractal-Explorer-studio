...
`fractal3d` — одиннадцать трёхмерных режимов: дистанционные оценки на GPU не дают пустой кадр,
...
`picker-marker` — отдельная быстрая проверка зелёной 3D-минисферы выбора C и того, что она не влияет на зонд поверхности,
