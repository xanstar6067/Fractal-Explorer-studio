...
`attractors [папка PNG]` — шесть объёмных систем, их кадры, JSON и параметры окна; `picker-marker` — отдельная быстрая проверка зелёной 3D-минисферы выбора C и того, что она не влияет на зонд поверхности,
