...
| `Views/Fractal3DPaletteWindow.xaml` / `.xaml.cs` | Менеджер палитр девяти трёхмерных фракталов с дистанционной оценкой: двенадцать встроенных палитр и пользовательские (`Palettes\custom_palettes_fractal3d.json`). Редактор задаёт опорные цвета (до 16), плавный градиент или полосы, гамму и, по желанию, окружение — фон вверху и внизу, цвет источника света и цвет материала. Рядом с цветами идёт живой кадр открытого вида. Вызывается из `Fractal3DWindow`. |
